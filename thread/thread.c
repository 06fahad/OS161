...
struct process* process_array[64];
...
{	
	...
	//kprintf("point 2"); //for debugging

	// If you add things to the thread structure, be sure to initialize
	// them here.
	int i;

	//kprintf("point 3"); //for debugging
	...
	for (i = 0; i < MAX_PROC; i++) {
		if (process_array[i] == NULL) {
			...
			lock_acquire (process_array[i]->exit_lock);
			//kprintf ("exit status: %d\n", process_array[i]->exit_lock->held);
			V(array_mut);
			return thread;
		}
	}
...
{
	array_mut = sem_create("array", 1);
	struct thread *me;
	//process_array = array_create();
	if(process_array==NULL){
		kprintf("Did not work process");
	}
...
int
thread_fork_mod(const char *name, 
	    void *data1, struct addrspace *data2,
